...
const initialEditLink = { id: null, titulo: '', url: '' }
...
export default function UserDashboard() {
  ...
  const [profile, setProfile] = useState({ nombre: '', bio: '', avatar_url: '', template: 'neonPulse', slug: '' })
  const [newLink, setNewLink] = useState(initialLink)
  const [editingLink, setEditingLink] = useState(initialEditLink)
  ...
  const startEditingLink = (link) => {
    setEditingLink({ id: link.id, titulo: link.titulo, url: link.url })
  }

  const cancelEditingLink = () => {
    setEditingLink(initialEditLink)
  }

  const saveEditingLink = async (id) => {
    if (!editingLink.titulo.trim() || !editingLink.url.trim()) {
      toast.error('Debes completar título y URL')
      return
    }
    ...
    try {
      const res = await fetch(`/api/links/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ titulo: editingLink.titulo, url: editingLink.url })
      })

      const payload = await res.json()
      if (!res.ok) throw new Error(payload.error || 'No se pudo actualizar el enlace')
  ...
  const effectiveSlug = profile.slug?.trim() || data.user.slug
  const publicUrl = origin ? `${origin}/${effectiveSlug}` : `/${effectiveSlug}`
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <div className="mt-3 space-y-1">
              <label className="text-xs font-medium text-gray-700">Tu nombre de usuario (slug):</label>
              <div className="flex gap-2 items-center">
                ...
                />
                <span className="text-xs text-gray-500">({publicUrl})</span>
              </div>
              <p className="text-xs text-gray-500">Vista pública: <a className="text-blue-600 hover:underline" href={publicUrl} target="_blank" rel="noreferrer">{publicUrl}</a></p>
            </div>
          </div>
                ...
                {editingLink.id === link.id ? (
                  <div className="space-y-2">
                    <input
                      ...
                      onChange={(e) => setEditingLink((prev) => ({ ...prev, titulo: e.target.value }))}
                      placeholder="Título"
                    />
                    <input
                      className="w-full border rounded-lg px-3 py-2"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-3">
                      <button type="button" onClick={() => startEditingLink(link)} className="text-blue-600">Editar</button>
                      <button type="button" onClick={() => removeLink(link.id)} className="text-red-600">Eliminar</button>
                    </div>
                  </div>
                )}
              </div>
