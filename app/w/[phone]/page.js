...
export default function WhatsAppPage({ params, searchParams }) {
  ...
  // WhatsApp Web - URL normal, TikTok NO la bloquea
  const waWebUrl = 'https://web.whatsapp.com/send?phone=' + cleanPhone + '&text=' + encodeURIComponent(text)

  useEffect(() => {
    // Redirigir inmediatamente a WhatsApp Web
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #25d366, #128c7e)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ background: 'white', borderRadius: '24px', padding: '40px 32px', maxWidth: '400px', width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        ...
        <a
          href={waWebUrl}
          style={{ display: 'block', background: '#25d366', color: 'white', fontWeight: '800', fontSize: '18px', padding: '16px', borderRadius: '14px', textDecoration: 'none', boxShadow: '0 8px 20px rgba(37,211,102,0.4)' }}
        >
          💬 Abrir WhatsApp
        </a>
