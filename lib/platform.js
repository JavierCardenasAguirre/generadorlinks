...
export async function syncSubscriptionStatus(usuarioId) {
  ...
  if (sub.status === 'trial' && new Date(sub.trial_ends_at).getTime() < now) {
    nextStatus = 'past_due'
  }

  if (sub.status === 'active' && new Date(sub.current_period_ends_at).getTime() < now) {
    nextStatus = 'past_due'
  }
  ...
  if (nextStatus !== sub.status) {
    ...
    return updated || { ...sub, status: nextStatus }
  }
...
export function getPublicUserPayload(userRow) {
  return {
    ...
    template: userRow.template || DEFAULT_TEMPLATE
  }
}
